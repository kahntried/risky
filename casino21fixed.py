...
< / style >
    """
                                                                                                            ...
                                                                                                            def play_sound(sound_type):
                                                                                                                ...
                                                                                                                return f'<div style="opacity: 0;">{sounds.get(sound_type, "")}</div>'
                                                                                                            ...
                                                                                                            class BlackjackGame:
                                                                                                                ...
                                                                                                                def dealer_play(self):
                                                                                                                    # Add initial delay before dealer starts
                                                                                                                    time.sleep(0.8)
                                                                                                            
                                                                                                                    while self.calculate_hand_value(self.dealer_hand) < 17:
                                                                                                                        self.dealer_hand.append(self.deal_card())
                                                                                                                        # Small delay between each dealer card for realism
                                                                                                                        if self.calculate_hand_value(self.dealer_hand) < 17:
                                                                                                                            time.sleep(0.4)  # Pause between cards
                                                                                                            ...
                                                                                                            with left_col:
                                                                                                                ...
                                                                                                                selected_theme = st.selectbox("", theme_options, index=theme_options.index(st.session_state.theme), key="theme_select")
                                                                                                                ...
                                                                                                                if total_games > 0:
                                                                                                                    ...
                                                                                                                    st.markdown(f"**Net:** <span style='color: {color};'>{symbol} ${total_winnings:,}</span>", unsafe_allow_html=True)
                                                                                                                else:
...
if not game.bet_placed and not game.game_over:
    ...
    with col1:
        if st.button("$50", key="bet_50", use_container_width=True):
            if game.place_bet(50):
                st.markdown(play_sound("chip"), unsafe_allow_html=True)
                st.rerun()
    ...
    custom_bet = st.number_input("Custom Amount", min_value=50, max_value=min(1000, game.balance), step=50, key="custom_bet")
...
if game.bet_placed and not game.player_hand and not game.dealer_hand:
    st.markdown(play_sound("deal"), unsafe_allow_html=True)
...
if not game.game_over and not game.player_stood:
    # Show actions if we have cards (either regular hand or split hands)
...
# Show dealer playing message when dealer is taking cards
elif game.player_stood and not game.game_over:
    st.markdown('<div style="text-align: center; color: #FFD700; font-size: 1.3rem; margin: 20px 0;">🎴 Dealer playing...</div>', unsafe_allow_html=True)

# Game result
if game.game_over:
    if game.game_result == "player_blackjack":
        st.markdown(play_sound("blackjack"), unsafe_allow_html=True)
        st.markdown('<div class="win-message">🎉 BLACKJACK! 🎉</div>', unsafe_allow_html=True)
        winnings = int(game.current_bet * 1.5)
        st.markdown(f'<div style="text-align: center; color: #32CD32; font-size: 1.2rem;">21! +${winnings} (3:2 payout)</div>', unsafe_allow_html=True)
        st.session_state.stats['wins'] += 1
        st.session_state.stats['total_winnings'] += winnings
    elif game.game_result == "dealer_blackjack":
        st.markdown(play_sound("lose"), unsafe_allow_html=True)
        st.markdown('<div class="lose-message">💔 DEALER BLACKJACK</div>', unsafe_allow_html=True)
        st.markdown(f'<div style="text-align: center; color: #FF6B6B;">Dealer 21! -${game.current_bet}</div>', unsafe_allow_html=True)
        st.session_state.stats['losses'] += 1
        st.session_state.stats['total_winnings'] -= game.current_bet
    elif game.game_result == "push_blackjack":
        st.markdown('<div style="font-size: 1.5rem; font-weight: bold; color: #FFA500; text-align: center;">🤝 BOTH BLACKJACK!</div>', unsafe_allow_html=True)
        ...
        st.markdown(f'<div style="text-align: center; color: #FF6B6B;">-${game.current_bet}</div>', unsafe_allow_html=True)
        st.session_state.stats['losses'] += 1
        st.session_state.stats['total_winnings'] -= game.current_bet
    elif game.game_result == "dealer_bust":
        st.markdown(play_sound("win"), unsafe_allow_html=True)
        st.markdown('<div class="win-message">🎉 DEALER BUSTS!</div>', unsafe_allow_html=True)
        st.markdown(f'<div style="text-align: center; color: #32CD32;">+${game.current_bet}</div>', unsafe_allow_html=True)
        st.session_state.stats['wins'] += 1
        st.session_state.stats['total_winnings'] += game.current_bet
        ...
        st.markdown('<div style="font-size: 1.5rem; font-weight: bold; color: #FFA500; text-align: center;">🤝 PUSH!</div>', unsafe_allow_html=True)
        st.markdown(f'<div style="text-align: center; color: #FFA500;">+$0</div>', unsafe_allow_html=True)
        st.session_state.stats['pushes'] += 1

    if game.balance < 50:
        st.markdown('<div style="font-size: 1.8rem; font-weight: bold; color: #FF0000; text-align: center; animation: pulse 2s infinite;">💸 BANKRUPT! 💸</div>', unsafe_allow_html=True)
...
# Auto-refresh for dealer animation with proper timing
if game.player_stood and not game.game_over:
# Faster refresh during dealer play to show each card
time.sleep(0.3)
st.rerun()